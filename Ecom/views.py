...
def customer_signup_view(request):
    if request.method=='POST':
        ...
        filename = fs.save('grocery/profile_pic/'+profile_pic.name, profile_pic)
        customer=Customer()
...
def admin_add_product_view(request):
    if request.method == 'POST':
        ...
        filename = fs.save('grocery/product_image/'+product_image.name, product_image)
        products=Product()
        products.name =request.POST.get('name')
        products.description =request.POST.get('description')
        products.gm =request.POST.get('gm')
        products.kg =request.POST.get('kg')
        products.lb =request.POST.get('lb')
...
def update_product_view(request,pk):
    ...
    if request.method == 'POST':
        update_products=Product.objects.get(id=pk)
        update_products.name =request.POST.get('name')
        update_products.description =request.POST.get('description')
        update_products.gm =request.POST.get('gm')
        update_products.kg =request.POST.get('kg')
        update_products.lb =request.POST.get('lb')
        ...
        if len(request.FILES) != 0:
            ...
            Product.objects.filter(id=pk).update(product_image=filename)
            return redirect('admin-products')
    return render(request, 'ecom/admin_update_product.html',{'products':products})
...
def payment_success_view(request):
    ...
    for product in products:
        Orders.objects.get_or_create(customer=customer,product=product,status='Pending',email=email,mobile=mobile,address=address)
...
def edit_profile_view(request):
    if request.method=='POST':
        customer=Customer.objects.filter(user_id=request.user.id).update(
        ...
        if len(request.FILES) != 0:
            ...
            Customer.objects.filter(user_id=request.user.id).update(profile_pic=filename)
